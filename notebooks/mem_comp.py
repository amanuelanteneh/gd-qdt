...
for N in tqdm(Ns, desc="Performing memory experiments"):
    # PNR detector POVM
    M = 608
    true_povm = pnr_povm(hilbert_dim=M, N=M, eta=eta)
    ...
    mem = memory_usage( (learn_phase_insensitive_povm, (logits, hyperparams, dataset, lam_smoothing, False)), interval=0.1, max_usage=True, include_children=not True)  
    mem_gd.append((mem - base_line) / 1000)  # mem is in Mb divide by 1k to get in Gb

    targets, probes = targets.cpu().numpy(), probes.cpu().numpy()
    base_line = memory_usage(timeout=1)[0]
    mem = memory_usage( (phase_insensitive_loss_cvx, (targets, probes, lam_smoothing, "MOSEK")), interval=0.1, max_usage=True, include_children=not True)
